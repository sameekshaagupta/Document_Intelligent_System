...
class RAGEngine:
    ...
    def clean_extracted_text(self, text):
        ...
        # Remove or replace common PDF artifacts
        replacements = {
            '♂': '',  # Remove male symbol
            '♀': '',  # Remove female symbol
        ...
        # Add spaces around email patterns
    ...
    def extract_text_from_file(self, file_path, file_type):
        ...
        try:
            if file_type.lower() == 'txt':
                ...
                pages_count = 1  # Simplified for docx
                paragraphs = []
    ...
    def extract_pdf_text_advanced(self, file_path):
        ...
        # Method 1: Try PyMuPDF (fitz) first - usually better quality
        try:
            import fitz
        ...
        # Method 2: Try pdfplumber for better layout preservation
        try:
            import pdfplumber
    ...
    def chunk_text(self, text, chunk_size=500, overlap=50):
        ...
        # Try to split by logical sections first (for resumes)
    ...
    def query_documents(self, document_id, question, num_chunks=3):
        ...
        try:
            ...
            # Get document to verify it exists and is processed
            try:
            ...
            # Perform similarity search with proper metadata filtering
            try:
                results = self.collection.query(
                    query_embeddings=[question_embedding.tolist()],
                    n_results=min(num_chunks, chunk_count),
                    where={"document_id": document_id}  # Fixed: Use simple metadata filtering
                )
            except Exception as e:
                print(f"ChromaDB query error: {str(e)}")
                # Fallback: query without filtering
                results = self.collection.query(
                    query_embeddings=[question_embedding.tolist()],
                    n_results=min(num_chunks, chunk_count)
                )
...
Answer:"""
            ...
            try:
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",  # or your LM Studio model
